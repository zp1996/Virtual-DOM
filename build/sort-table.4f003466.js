...
function Merge(arr, left, mid, right, temp, key) {
	var i = left,
	    ...
	    k = 0;
	while (i < mid && j < n) {
		if (arr[i][key] <= arr[j][key]) temp[k++] = arr[i++];else temp[k++] = arr[j++];
	}
	while (i < mid) {
		temp[k++] = arr[i++];
	}while (j < n) {
...
function MergeSort(arr, key) {
	...
	for (var i = 1; i < len; i *= 2) {
		for (var j = 0; j < len; j += i * 2) {
			if (j + 2 * i < len) Merge(arr, j, j + i, j + 2 * i, temp, key);else if (j + 2 * i > len && j + i <= len) Merge(arr, j, j + i, len, temp, key);
		}
	}
}
/* harmony default export */ __webpack_exports__["a"] = (MergeSort);
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__MergeSort__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__lib_index__ = __webpack_require__(2);



var Element = __WEBPACK_IMPORTED_MODULE_1__lib_index__["default"].Element,
    diff = __WEBPACK_IMPORTED_MODULE_1__lib_index__["default"].diff,
    patch = __WEBPACK_IMPORTED_MODULE_1__lib_index__["default"].patch;


var data = [{ uid: 1, username: 'xiaoming', age: 20, height: 178 }, { uid: 2, username: 'xiaohong', age: 48, height: 165 }, { uid: 3, username: 'xiaohua', age: 18, height: 170 }, { uid: 4, username: 'xiaogang', age: 59, height: 160 }, { uid: 5, username: 'kobe', age: 38, height: 196 }];
var $ = function $(id) {
...
(function () {
	...
	function tradBurnTbody() {
		var tradition = $("tradition-way"),
		    html = "";
		for (var i = 0, item; item = data[i++];) {
			html += "<tr><td>" + item.uid + "</td><td>" + item.username + "</td><td>" + item.age + "</td><td>" + item.height + "</td></tr>";
		}
	...
	for (var i = 0, len = sorts.length; i < len; i++) {
		(function (i) {
			sorts[i].addEventListener("click", function () {
				...
				__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__MergeSort__["a" /* default */])(data, key);
				// render tbody
				tradBurnTbody();
				// handle thead
...
(function () {
	...
	function formTh() {
		return keys.map(function (val, i) {
			if (val === "username") return new Element("th", [val]);
	...
	function formTd() {
		return data.map(function (val) {
			return new Element("tr", { key: val.uid }, [new Element("td", [val.uid]), new Element("td", [val.username]), new Element("td", [val.age]), new Element("td", [val.height])]);
		});
	}
	var table = new Element("table", [new Element("thead", [new Element("tr", ths)]), new Element("tbody", trs)]),
		...
		for (var i = 0, len = sort.length; i < len; i++) {
			(function (i) {
				sort[i].addEventListener("click", function () {
					...
					ths = formTh();
					// sort data
					__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_0__MergeSort__["a" /* default */])(data, this.getAttribute("sort-key"));
					// update tbody
					trs = formTd();
					// form the new tree
					var newTable = new Element("table", [new Element("thead", [new Element("tr", ths)]), new Element("tbody", trs)]);
					// get patches
